...
import {
  ...
  Snackbar, Alert, Container, Grid, FormControl
} from '@mui/material';
...
export function UpdatePasswordForm(): React.JSX.Element {
  ...
  const [editFood, setEditFood] = useState<Partial<Food>>({});
  const [viewOpen, setViewOpen] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  const [page, setPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');
  const [showSuccess, setShowSuccess] = useState(false);
  const [confirmDeleteOpen, setConfirmDeleteOpen] = useState(false); // New state for confirmation dialog
  const [foodToDelete, setFoodToDelete] = useState<Food | null>(null); // Track food to delete
  const rowsPerPage = 5;
